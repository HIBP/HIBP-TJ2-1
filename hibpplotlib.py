...
def plot_3d(B, volume_corner1, volume_corner2,
...
# %% matplotlib plot 3D
def plot_3dm(B, points, cutoff=2):
    '''
...
# %%
def plot_geometry(ax, camera_data_filename='TJII_camera.dat',
                  separatrix_data_filename='TJII_lcfs.dat',
                  major_radius=1.5, plot_sep=True):
    '''
    plot camera and separatrix in XY plane
    '''
    # get TJ-II camera and plasma contours
    camera = np.loadtxt(camera_data_filename)
...
# %% Plot trajectories
def plot_traj(traj_list, geom, Ebeam, UA2, config, full_primary=False,
    ...
    for tr in traj_list:
        if tr.Ebeam == Ebeam and tr.U['A2'] == UA2:
            ...
            ax1.set_title('E={} keV, UA2={} kV, {}'.format(Ebeam, UA2, config))
...
# %%
def plot_fan(traj_list, geom, Ebeam, UA2, config, plot_traj=True,
...
# %%
def plot_scan(traj_list, geom, Ebeam, config, full_primary=False,
    ...
    for i_E in range(0, N_E, 1):
        ...
        for i_tr in range(len(traj_list)):
            if traj_list[i_tr].Ebeam == Elist[i_E]:
                ...
                if reflect:
                    # reflect the point to HIBP plane
                    reflected_xyz = hb.reflect(np.array([x, y, z]))
                    E_grid[k, :, i_E] = reflected_xyz
                else:
                    E_grid[k, :, i_E] = [x, y, z]
...
# %%
def plot_grid_a3b3(traj_list, geom, config,
                   linestyle_A2='--', linestyle_E='-',
                   marker_E='p'):
    '''
...
# %%
def plot_traj_toslits(traj_list, geom, config, Ebeam=127.,
...
def plot_fat_beam(fat_beam_list, geom, config, slits=[2], scale=3):
    ...
    set_axes_param(ax3, 'Z (m)', 'Y (m)')
    tr = fat_beam_list[0]
    ax1.set_title('E={} keV, UA2={} kV, {}'.format(tr.Ebeam, tr.U['A2'], config))
...
def plot_svs(fat_beam_list, geom, config, slits=[2],
    ...
    geom.plot(ax1, axes='XY', plot_aim=False,
              plot_sep=plot_sep, plot_analyzer=True)
    geom.plot(ax3, axes='ZY', plot_aim=False, plot_analyzer=True)
...
# %%
def plot_sec_angles(traj_list, config, Ebeam='all'):
    '''
