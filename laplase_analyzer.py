...
if __name__ == '__main__':
    ...
    if plts_name == 'A1':
        beamline = 'prim'
        length = 0.11  # along X [m]
        width = 0.11  # along Z [m]
        thick = 0.004  # [m]
        gap = 0.03  # distance between plates along Y [m]
        ...
        beamline = 'prim'
        length = 0.16  # along X [m]
        width = 0.11  # along Z [m]
        thick = 0.004  # [m]
        gap = 0.03  # distance between plates along Y [m]
        alpha_sw = 7.5  # sweep angle [deg] for flared plates
        l_sw = 0.04 / np.cos(np.radians(alpha_sw))  # length of a flared part
        ...
        thick = 0.004  # [m]
        gap = 0.06  # distance between plates along Y [m]
        Uupper_plate = 1e3
        Ulower_plate = 0.
        ...
        n_slits, slit_dist, slit_w = 2, 0.032, 7e-3
        # analyzer geometry
        theta_an = 30.
        width = 0.2  # along Z [m]
        thick = 0.006  # 0.01  # [m]
        gap = 0.079  # distance between plates along Y [m]

        # YD1 = 0.02 + thick + np.cos(theta_an*drad) * (n_slits//2 * slit_dist
        #                                               + 0.5*slit_w)
        YD1 = thick + 0.063
        YD2 = thick + 0.0847  # YD1
        YD = YD1 + YD2
        XD = 0.548  # 3 * np.sqrt(3) * YD

        length = 0.80  # 1.2 * XD  # along X [m]
